...
pub fn check_level3(binary: &str, args: &[String]) -> Option<Threat> {
    let b = bn(binary);
    match b {
        "python3" | "python" => l3_lang(args, "-c", &["shutil.rmtree","os.system","os.popen","subprocess.call","subprocess.run","subprocess.Popen","os.remove","os.unlink"], "Python"),
        "node" => l3_lang(args, "-e", &["child_process.exec","child_process.spawn","fs.rm","fs.rmSync","fs.rmdir","fs.unlink","process.exit"], "Node"),
        "perl" => l3_lang(args, "-e", &["system(","exec(","unlink(","rmdir(","`rm"], "Perl"),
        "ruby" => l3_lang(args, "-e", &["system(","exec(","FileUtils.rm_rf","FileUtils.rm_r","File.delete"], "Ruby"),
        "php" => l3_lang(args, "-r", &["system(","exec(","passthru(","shell_exec(","unlink(","rmdir("], "PHP"),
...
fn l3_ansible(args: &[String]) -> Option<Threat> {
    if args.iter().any(|a| a == "-m") && args.iter().any(|a| a == "shell" || a == "command") {
        if let Some(pos) = args.iter().position(|a| a == "-a") {
            if let Some(cmd) = args.get(pos + 1) { return bash_ast(cmd); }
        }
    }
    None
}
