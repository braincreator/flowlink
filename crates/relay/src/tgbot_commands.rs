...
pub async fn handle_command(
    ...
    match cmd {
        Command::Start => self::cmd_start(bot, msg, ctx).await,
        Command::Help => self::cmd_help(bot, msg, ctx).await,
        Command::Status => self::cmd_status(bot, msg, ctx).await,
        Command::Servers => self::cmd_servers(bot, msg, ctx).await,
        Command::Plans => self::cmd_plans(bot, msg, ctx).await,
        Command::Billing => self::cmd_billing(bot, msg, ctx).await,
        Command::Myplan => self::cmd_myplan(bot, msg, ctx).await,
        Command::Subscribe => {
            let plan_id = msg.text()
                .and_then(|t| t.strip_prefix("/subscribe"))
                .unwrap_or("")
            ...
            self::cmd_subscribe(bot, msg, ctx).await
        }
        Command::Invoices => self::cmd_invoices(bot, msg, ctx).await,
        Command::Usage => self::cmd_usage(bot, msg, ctx).await,
        Command::Devices => self::cmd_devices(bot, msg, ctx).await,
        Command::Shield => self::cmd_shield(bot, msg, ctx).await,
        Command::Logs => self::cmd_logs(bot, msg, ctx).await,
        Command::Approvals => self::cmd_approvals(bot, msg, ctx).await,
        Command::Config => self::cmd_config(bot, msg, ctx).await,
        Command::Reload => self::cmd_reload(bot, msg, ctx).await,
        Command::Emergency => self::cmd_emergency(bot, msg, ctx).await,
        Command::Exec => self::cmd_exec(bot, msg, ctx).await,
        Command::Backups => self::cmd_backups(bot, msg, ctx).await,
        Command::Restore => self::cmd_restore(bot, msg, ctx).await,
        Command::Policy => self::cmd_policy(bot, msg, ctx).await,
        Command::Settings => self::cmd_settings(bot, msg, ctx).await,
        Command::Notiftest => self::cmd_notiftest(bot, msg, ctx).await,
        Command::Substatus => self::cmd_substatus(bot, msg, ctx).await,
        Command::Subcancel => self::cmd_subcancel(bot, msg, ctx).await,
        Command::Subchange => self::cmd_subchange(bot, msg, ctx).await,
    }
}
...
#[derive(teloxide::utils::command::BotCommands)]
...
pub enum Command {
    ...
    Settings,
    #[command(description = "Тест уведомления")]
    Notiftest,
    #[command(description = "Статус подписки")]
    Substatus,
...
// ═══════════════════════════════════════════════
// Update processing (webhook handler entry point)
// ═══════════════════════════════════════════════
...
pub async fn process_update(
    ...
    if let Some(ref msg) = msg {
        if let Some(text) = msg.text() {
            if let Ok(cmd) = Command::parse(text, "flowlinkbot") {
                if let Err(e) = handle_command(bot, msg.clone(), cmd, ctx).await {
    ...
    match &update {
        teloxide::types::Update {
            ...
            if let Err(e) = handle_callback(bot, q.clone(), ctx).await {
