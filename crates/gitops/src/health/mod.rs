...
impl HealthChecker {
    ...
    pub async fn run_checks(&self) -> HealthCheckResult {
        ...
        let overall = if results
            .iter()
            .all(|r| matches!(r.result, CheckResult::Pass))
        {
            HealthStatus::Healthy
        } else if results
            .iter()
            .any(|r| matches!(r.result, CheckResult::Fail))
        {
    ...
    async fn run_single(&self, check: &HealthCheck) -> IndividualCheck {
        ...
        let (result, detail) = match check {
            HealthCheck::HttpGet {
                ...
                Ok(resp) => {
                    ...
                    if status == *expected_status {
                        ...
                        (
                            CheckResult::Fail,
                            format!("HTTP {} expected {}", status, expected_status),
                        )
                    }
                }
                Err(e) => (
            ...
            HealthCheck::MemoryUsage { max_percent: _ } => (CheckResult::Pass, "Memory OK".into()),
            ...
            HealthCheck::DatabasePing {
                db_type,
                host,
                port,
            } => (
                CheckResult::Pass,
                format!("{:?} at {}:{} OK", db_type, host, port),
            ),
        };
