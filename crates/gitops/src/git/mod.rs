        ...
        let repository = spawn_blocking(move || -> Result<Repository> {
            if repo_path.exists() {
                ...
                let repo = Repository::init(&repo_path)
                    .with_context(|| format!("Failed to initialize repository at {:?}", repo_path))?;
        ...
        spawn_blocking({
            ...
            move || -> Result<Oid> {
                ...
                let full_message = if let Some(meta) = metadata {
                    format!("{}\n\nChange-Type: {:?}\nMetadata: {}", 
                           message, change_type, serde_json::to_string(&meta)?)
                } else {
                ...
                info!("Created commit {} for change type {:?}", commit_id, change_type);
                Ok(commit_id)
            }
        }).await?
    }
        ...
        spawn_blocking({
            ...
            move || -> Result<Vec<CommitInfo>> {
                ...
                for (count, oid) in revwalk.enumerate() {
                    ...
                    let author_email = commit.author().email().unwrap_or("unknown@local").to_string();
...
mod tests {
    ...
    fn create_test_config() -> GitConfig {
        ...
        GitConfig {
            repo_path: temp_dir.path().join("test-repo").to_string_lossy().to_string(),
            remote_url: None,
    ...
    async fn test_create_snapshot() {
        ...
        let commit_id = engine.commit_state_change(
            "Initial commit", 
            ChangeType::StateUpdate, 
            None
        ).await.unwrap();
        
        let snapshot_tag = engine.create_snapshot(BackupType::StateSnapshot).await.unwrap();
