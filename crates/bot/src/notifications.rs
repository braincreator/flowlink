...
pub async fn notify(bot: &Bot, state: &Arc<BotState>, message: &str) {
...
pub async fn agent_connected(bot: &Bot, state: &Arc<BotState>, agent_id: &str, hostname: &str) {
    notify(bot, state, &format!("🟡 <b>Сервер подключён</b>\n\n🖥 <b>{}</b>\nID: <code>{}</code>", hostname, agent_id)).await;
}

/// Notify about agent disconnection
pub async fn agent_disconnected(bot: &Bot, state: &Arc<BotState>, agent_id: &str, hostname: &str) {
    notify(bot, state, &format!("🔴 <b>Сервер отключён</b>\n\n🖥 <b>{}</b>\nID: <code>{}</code>", hostname, agent_id)).await;
}

/// Notify about security alert
pub async fn shield_alert(bot: &Bot, state: &Arc<BotState>, agent_id: &str, risk: &str, command: &str) {
...
pub async fn payment_event(bot: &Bot, state: &Arc<BotState>, account_id: &str, event: &str, amount: &str) {
    notify(bot, state, &format!(
        "💳 <b>Платёж</b>\n\n📊 {}\n💰 {}\n👤 <code>{}</code>",
...
pub async fn approval_request(bot: &Bot, state: &Arc<BotState>, approval_id: &str, agent_id: &str, command: &str, risk: &str) {
