...
//!
//! ```ignore
//! use flowlink_billing::plan_gate::{require_feature, check_limit, PlanGateError};
//!
//! async fn handler(plan: &Option<Plan>) -> impl IntoResponse {
//!     require_feature(plan, "approval", Some(feature_min_tier("approval")))?;
//!     check_limit(plan, "max_agents", current_agents, Some(limit_min_tier("max_agents")))?;
//!     // ... handler logic
//! }
//! ```
...
mod tests {
    ...
    fn make_plan() -> Plan {
        Plan {
            ...
            features: PlanFeatures {
                ...
                ai_ops: false, change_management: false, redaction: false,
                serverguard: false, serverguard_level: String::new(),
            },
            limits: PlanLimits {
    ...
    fn make_starter_plan() -> Plan {
        Plan {
            ...
            available: true, legacy: false, trial_days: Some(14),
            billing_period: "month".into(),
        }
    }
