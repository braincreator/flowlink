...
/// Re-export feature_min_tier from plan_gate (single source of truth).
pub use crate::plan_gate::feature_min_tier;

/// Check if a feature is enabled on the plan.
fn has_feature(plan: &Plan, feature: &str) -> bool {
    match feature {
        ...
        "redaction" => plan.features.redaction,
...
fn get_limit(plan: &Plan, limit: &str) -> u64 {
    match limit {
...
/// Does NOT require State — plan comes from request extensions.
...
/// Helper macro for handlers that need to check a limit dynamically.
#[macro_export]
...
mod tests {
    use super::*;
    use crate::plans::{PlanFeatures, PlanLimits};

    fn make_starter() -> Plan {
        Plan {
            id: "starter".into(), name: "Starter".into(), description: "Free".into(),
            tier: 0, price_kopecks: 0, annual_price_kopecks: None, annual_discount_percent: 0,
            features: PlanFeatures {
                shield: true, shield_level: "basic".into(), mcp_gateway: true, policy_engine: true,
                approval: false, rbac: false, pattern_learning: false, e2ee: true, audit_log: true,
                webhooks: false, siem_export: false, sso: false, on_premise: false,
                forensics: false, service_catalog: false, ai_ops: false, change_management: false,
                redaction: false, serverguard: false, serverguard_level: String::new(),
            },
            limits: PlanLimits {
                max_agents: 1, max_users: 1, audit_retention_days: 30, api_rate_limit: 100,
                api_rate_window_secs: 60, max_custom_rules: 3, max_policies: 1, max_webhooks: 0,
                approval_channels: vec![], siem_formats: vec![], allowed_shield_levels: vec![],
                support_tier: "community".into(),
            },
            available: true, legacy: false, trial_days: None, billing_period: "month".into(),
        }
    }

    fn make_pro() -> Plan {
        Plan {
            id: "team".into(), name: "Team".into(), description: "For dev teams".into(),
            tier: 2, price_kopecks: 199000, annual_price_kopecks: Some(1910400), annual_discount_percent: 20,
            features: PlanFeatures {
                shield: true, shield_level: "advanced".into(), mcp_gateway: true, policy_engine: true,
                approval: true, rbac: true, pattern_learning: true, e2ee: true, audit_log: true,
                webhooks: true, siem_export: true, sso: false, on_premise: false,
                forensics: true, service_catalog: false, ai_ops: false, change_management: false,
                redaction: true, serverguard: true, serverguard_level: "basic".into(),
            },
            limits: PlanLimits {
                max_agents: 5, max_users: 5, audit_retention_days: 60, api_rate_limit: 500,
                api_rate_window_secs: 60, max_custom_rules: 50, max_policies: 5, max_webhooks: 3,
                approval_channels: vec!["telegram".into()], siem_formats: vec!["json".into()],
    ...
    fn test_get_limit() {
        ...
        assert_eq!(get_limit(&plan, "unknown"), 0);
    }
    ...
    fn test_route_requirements_coverage() {
        let paths = [
            "/api/approvals", "/api/approvals/123/approve",
            "/api/v1/policies", "/api/v1/policies/123",
            "/api/v1/patterns/apply", "/auth/saml/login",
        ];
        for path in &paths {
    ...
    fn test_gate_blocked_serialization() {
        let blocked = GateBlocked {
            error: "feature_not_available", feature: Some("approval"), limit: None,
            required_plan: Some("team"), upgrade_url: Some("/pricing?upgrade=team".into()),
