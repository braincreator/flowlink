...
fn parse_saml_response(response_b64: &str) -> Result<SamlAssertion, String> {
    ...
    let xml_str = String::from_utf8(xml_bytes).map_err(|e| format!("UTF-8 decode error: {e}"))?;
    ...
    for attr_xml in extract_all_tags(&xml_str, "saml:Attribute")
        .into_iter()
...
pub async fn saml_login(
    ...
    let url = format!(
        ...
        config.idp_sso_url,
        crate::urlencoding::encode(&encoded),
        crate::urlencoding::encode(&relay_state),
    );
...
pub async fn saml_acs(
    ...
    let (account_id, is_admin) = match &state.auth_engine {
        ...
        None => {
            return (axum::http::StatusCode::INTERNAL_SERVER_ERROR, "Auth not configured").into_response()
        }
    };
...
pub async fn saml_metadata(Extension(state): Extension<Arc<AuthState>>) -> impl IntoResponse {
    let config = match &state.saml_config {
        Some(c) => c.lock().await.clone(),
        None => return (axum::http::StatusCode::NOT_FOUND, "SAML not configured").into_response(),
    };
