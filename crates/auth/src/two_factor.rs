...
/// Check if account has 2FA enabled, return temp token response if so.
/// Called from auth endpoints after successful credential verification.
pub fn check_2fa(
    account_id: &str,
    email: Option<&str>,
    name: Option<&str>,
...
pub async fn setup_2fa(
    ...
    if let Err(e) =
        flowlink_db::accounts::AccountRepo::set_totp_secret(pool, &claims.account_id, &secret_b32).await
    {
    ...
    let secret_b32 = match secret {
        ...
        None => {
            return (
                ...
                Json(json!({"error": "2FA not set up. Call /api/auth/2fa/setup first."})),
            )
                .into_response()
        }
    };
...
pub async fn disable_2fa(
    ...
    let secret_b32 = match secret {
        ...
        None => {
            return (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "No TOTP secret found"})),
            )
                .into_response()
        }
    };
...
pub async fn complete_2fa(
    ...
    let temp_claims = match validate_temp_token(&body.temp_token) {
        ...
        None => {
            return (
                StatusCode::UNAUTHORIZED,
                Json(json!({"error": "Invalid or expired temp token"})),
            )
                .into_response()
        }
    };
    ...
    let (totp_enabled, secret) =
        match flowlink_db::accounts::AccountRepo::get_totp(pool, &temp_claims.account_id).await {
            Ok(r) => r,
            Err(e) => {
                log::error!("Failed to get TOTP: {e}");
                return (
    ...
    if !totp_enabled {
        return (
            StatusCode::BAD_REQUEST,
            Json(json!({"error": "2FA not enabled for this account"})),
        )
            .into_response();
    }
    ...
    let secret_b32 = match secret {
        ...
        None => {
            return (
                StatusCode::BAD_REQUEST,
                Json(json!({"error": "No TOTP secret"})),
            )
                .into_response()
        }
    };
    ...
    match engine.create_tokens(
        ...
        temp_claims.avatar_url.as_deref(),
