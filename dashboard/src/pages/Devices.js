...
export default function Devices() {
    ...
    const { data: devices, loading, isLive, refresh } = useApi(() => api.getDevices(), mockDevices, { pollMs: 30000 });
    ...
    return (_jsxs("div", { className: "space-y-6 fade-in", children: [!isLive && (_jsx("div", { className: "rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3 text-sm text-amber-400", children: "\u26A0\uFE0F Connected to mock data. Start relay for live data." })), _jsxs("div", { className: "flex justify-between items-center", children: [_jsxs("div", { className: "text-sm text-[var(--color-dim)]", children: [devices.length, " paired devices"] }), _jsxs("button", { onClick: handlePair, className: "flex items-center gap-2 rounded-xl bg-[var(--color-accent)] px-4 py-2.5 text-sm font-medium text-white transition-all hover:bg-[var(--color-accent-light)]", children: [_jsx(Plus, { size: 16 }), " Pair Device"] })] }), _jsx(DataTable, { columns: [
