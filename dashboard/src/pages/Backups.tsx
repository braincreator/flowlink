  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)] p-5">
        <h3 className="mb-4 text-sm font-semibold text-[var(--color-dim)] uppercase tracking-wider">{t('backups.storage_usage')}</h3>
      ...
      <DataTable
        columns={[
          { key: 'id', label: t('backups.id'), render: (r: any) => <span className="font-mono text-xs">{r.id}</span> },
          { key: 'hostname', label: t('backups.agent'), render: (r: any) => (
            <div className="flex items-center gap-2"><HardDrive size={14} className="text-[var(--color-accent)]" />{r.hostname || r.agent_id || '—'}</div>
          )},
          { key: 'files', label: t('backups.files'), render: (r: any) => r.files ? (
          ...
          { key: 'size_bytes', label: t('backups.size'), render: (r: any) => r.size_bytes ? <span className="font-mono text-xs">{formatSize(r.size_bytes)}</span> : <span className="text-[var(--color-dim)]">—</span> },
          { key: 'timestamp', label: t('backups.time'), render: (r: any) => <span className="text-xs text-[var(--color-dim)]">{new Date(r.timestamp || r.timestamp_iso).toLocaleString()}</span> },
        ...
        data={backups} searchPlaceholder={t("backups.search_backups")}
      />

      <Modal open={!!restoreTarget} onClose={() => setRestoreTarget(null)} title={t('backups.confirm_restore')} actions={
        <>
          <button onClick={() => setRestoreTarget(null)} className="rounded-lg border border-[var(--color-border)] px-4 py-2 text-sm">{t('common.cancel')}</button>
          <button onClick={() => setRestoreTarget(null)} className="rounded-lg bg-amber-500 px-4 py-2 text-sm font-medium text-black hover:bg-amber-400">{t('backups.restore')}</button>
        </>
      }>
        {restoreTarget && (
          <div className="text-sm">
            <p className="text-[var(--color-dim)]">{t('backups.restore_from')} <span className="text-[var(--color-text)]">{new Date(restoreTarget.timestamp).toLocaleString()}</span>.</p>
            {restoreTarget.files && (
              <div className="mt-3 rounded-lg bg-[var(--color-bg)] p-3">
                <div className="text-xs text-[var(--color-dim)] mb-1">{t("backups.restore_files")}</div>
                {restoreTarget.files.map((f: string) => <div key={f} className="font-mono text-xs">{f}</div>)}
              </div>
            )}
            <p className="mt-3 text-amber-400 text-xs">{t("backups.restore_warning")}</p>
          </div>
        )}
      </Modal>
    </div>
