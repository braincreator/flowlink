  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <StatCard label="Backends" value="0" color="accent" icon={<Brain size={24} />} />
        <StatCard label="Health" value={healthy ? 'OK' : 'Degraded'} color={healthy ? 'green' : 'red'} icon={<Server size={24} />} />
        <StatCard label="Models" value="—" color="blue" icon={<Globe size={24} />} />
      </div>

      <div className="rounded-xl border border-[var(--color-border)] bg-[var(--color-surface)]">
        <div className="flex items-center justify-between border-b border-[var(--color-border)] px-6 py-4">
          <h3 className="font-semibold">LLM Backends</h3>
        ...
        {backends.length === 0 ? (
          <EmptyState
            icon={<Brain size={40} />}
            title="No LLM backends configured"
            description="Add an LLM backend via the API to see it here."
          />
        ) : (
          <DataTable
            columns={[
