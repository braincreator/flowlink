...
export function useSessionRecorder() {
  ...
  const recordSession = useCallback(async (
    ...
    return new Promise<void>(async (resolve, reject) => {
      try {
        ...
        const events: CastEvent[] = lines.slice(1)
          .map(l => JSON.parse(l))
          .filter((e: CastEvent) => e[1] === 'o');
    ...
    }) as unknown as Promise<RecordingResult>;
  }, []);
  ...
  const downloadRecording = useCallback((result: RecordingResult, filename?: string) => {
  ...
  const shareToTelegram = useCallback(async (result: RecordingResult, chatId?: string) => {
