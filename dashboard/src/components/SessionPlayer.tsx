...
export default function SessionPlayer({
  ...
  const startPlayback = useCallback((fromOffset = 0) => {
    if (!events || !termRef.current) return;
    stopPlayback();
  ...
  return (
    <div className={`rounded-xl border border-white/[0.06] bg-[#0d1117] overflow-hidden ${className}`}>
      {/* Terminal viewport */}
      <div ref={containerRef} className="min-h-[200px]" />

      {/* Controls */}
      {loaded && (
        <div className="flex items-center gap-3 border-t border-white/[0.06] bg-white/[0.02] px-3 py-2">
          <button
            onClick={() => {
            ...
            className="rounded-lg p-1.5 text-[var(--color-dim)] hover:bg-white/5 hover:text-white transition-colors"
          >
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (playing) startPlayback(val);
            }}
            className="flex-1 h-1 accent-[var(--color-accent)]"
          />

          {/* Time */}
          <span className="text-xs text-[var(--color-dim)] tabular-nums min-w-[80px] text-right">
            {formatTime(seek)} / {formatTime(events.totalDuration)}
          </span>
          ...
          <select
            ...
            className="rounded-lg bg-white/5 border-none text-xs text-[var(--color-dim)] px-2 py-1 outline-none"
          >
            {speedOptions.map(s => (
