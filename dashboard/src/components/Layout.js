...
export function DataTable({ columns, data, onRowClick, emptyText, searchPlaceholder }) {
    ...
    return (_jsxs("div", { children: [searchPlaceholder && (_jsx("div", { className: "mb-4", children: _jsx("input", { type: "text", placeholder: searchPlaceholder, value: search, onChange: e => { setSearch(e.target.value); setPage(0); }, className: "w-full max-w-xs rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] px-3 py-2 text-sm placeholder-[var(--color-dim)] focus:border-[var(--color-accent)] focus:outline-none transition-colors" }) })), _jsx("div", { className: "overflow-x-auto rounded-xl border border-[var(--color-border)]", children: _jsxs("table", { className: "w-full text-sm", children: [_jsx("thead", { children: _jsx("tr", { className: "border-b border-[var(--color-border)] bg-[var(--color-surface)]", children: columns.map(col => (_jsxs("th", { onClick: () => toggleSort(col.key), "aria-sort": sortKey === col.key ? (sortDir === 'asc' ? 'ascending' : 'descending') : undefined, className: `cursor-pointer px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-[var(--color-dim)] hover:text-[var(--color-text)] transition-colors ${col.className || ''}`, children: [col.label, " ", sortKey === col.key && (sortDir === 'asc' ? '↑' : '↓')] }, col.key))) }) }), _jsx("tbody", { children: paged.length === 0 ? (_jsx("tr", { children: _jsx("td", { colSpan: columns.length, className: "px-4 py-12 text-center text-[var(--color-dim)]", children: emptyText }) })) : paged.map((row, i) => (_jsx("tr", { onClick: () => onRowClick?.(row), className: `border-b border-[var(--color-border)] transition-colors hover:bg-[var(--color-surface2)] ${onRowClick ? 'cursor-pointer' : ''}`, children: columns.map(col => (_jsx("td", { className: `px-4 py-3 ${col.className || ''}`, children: col.render ? col.render(row) : String(row[col.key] ?? '') }, col.key))) }, i))) })] }) }), totalPages > 1 && (_jsxs("div", { className: "mt-4 flex items-center justify-between text-sm text-[var(--color-dim)]", children: [_jsxs("span", { children: [filtered.length, " ", t('common.results')] }), _jsxs("div", { className: "flex gap-2", children: [_jsx("button", { onClick: () => setPage(p => Math.max(0, p - 1)), disabled: page === 0, className: "rounded-lg border border-[var(--color-border)] px-3 py-1.5 text-xs hover:bg-[var(--color-surface2)] disabled:opacity-30 transition-colors", children: t('common.prev') }), _jsxs("span", { className: "flex items-center px-2", children: [page + 1, " / ", totalPages] }), _jsx("button", { onClick: () => setPage(p => Math.min(totalPages - 1, p + 1)), disabled: page >= totalPages - 1, className: "rounded-lg border border-[var(--color-border)] px-3 py-1.5 text-xs hover:bg-[var(--color-surface2)] disabled:opacity-30 transition-colors", children: t('common.next') })] })] }))] }));
}
...
export function TerminalOutput({ text }) {
    ...
    return (_jsxs("div", { className: "relative rounded-xl border border-[var(--color-border)] bg-[#0d0e14] p-4 font-mono text-sm", children: [_jsx("button", { onClick: copy, className: "absolute top-2 right-2 flex items-center gap-1 rounded-md bg-[var(--color-surface2)] px-2 py-1 text-xs text-[var(--color-dim)] hover:text-[var(--color-text)] transition-colors", children: copied ? _jsxs(_Fragment, { children: [_jsx(Check, { size: 12 }), " ", t('common.copied')] }) : _jsxs(_Fragment, { children: [_jsx(Copy, { size: 12 }), " ", t('common.copy')] }) }), _jsx("pre", { className: "whitespace-pre-wrap break-all text-[var(--color-text)] max-h-80 overflow-auto", children: text })] }));
}
export function EmptyState({ icon, title, description, action }) {
    return (_jsxs("div", { className: "flex flex-col items-center justify-center py-16 text-center", children: [_jsx("div", { className: "mb-4 opacity-30", children: icon }), _jsx("h3", { className: "text-lg font-semibold text-[var(--color-dim)]", children: title }), description && _jsx("p", { className: "mt-2 max-w-sm text-sm text-[var(--color-dim)] opacity-70", children: description }), action && _jsx("div", { className: "mt-4", children: action })] }));
}
export function LoadingSkeleton({ lines = 3 }) {
    return (_jsxs("div", { className: "space-y-3", role: "status", "aria-label": "Loading", children: [_jsx("span", { className: "sr-only", children: "Loading..." }), Array.from({ length: lines }, (_, i) => (_jsx("div", { className: "shimmer h-4 rounded", style: { width: `${60 + Math.random() * 40}%` } }, i)))] }));
}
export function Toast({ toasts, onRemove }) {
    const icons = { success: _jsx(Check, { size: 14, className: "text-emerald-400" }), error: _jsx(X, { size: 14, className: "text-rose-400" }), info: _jsx(AlertTriangle, { size: 14, className: "text-blue-400" }), warning: _jsx(AlertTriangle, { size: 14, className: "text-amber-400" }) };
    const colors = {
    ...
    return (_jsx("div", { className: "fixed bottom-6 right-6 z-[100] flex flex-col gap-2", "aria-live": "polite", children: toasts.map(toast => (_jsxs("div", { className: `flex items-center gap-3 rounded-xl border px-4 py-3 shadow-lg fade-in ${colors[toast.type]}`, children: [icons[toast.type], _jsxs("div", { children: [_jsx("div", { className: "text-sm font-medium", children: toast.title }), toast.message && _jsx("div", { className: "text-xs text-[var(--color-dim)]", children: toast.message })] }), _jsx("button", { onClick: () => onRemove(toast.id), className: "ml-2 text-[var(--color-dim)] hover:text-[var(--color-text)]", children: _jsx(X, { size: 14 }) })] }, toast.id))) }));
}
